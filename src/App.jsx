...
// ─── Constants ────────────────────────────────────────────────────────────────
const SPEND_CATS = ["Groceries","Transport","Dining","Beauty","Home","Entertainment","Clothing","Health","Travel","Other"];
const WISH_CATS  = ["Fashion","Beauty","Tech","Travel","Home","Wellness","Other"];

const DEFAULT_BUDGET = [
  { label:"Savings",        pct:20, color:T.sage  },
  { label:"Home & Bills",   pct:30, color:T.sand  },
  { label:"Transport",      pct:10, color:T.clay  },
  { label:"Groceries",      pct:15, color:T.mist  },
  { label:"Personal & Fun", pct:15, color:T.blush },
  { label:"Investments",    pct:10, color:T.teal  },
];
...
function impulseScore(days, quiz) {
  const d = Math.min(days/90, 1);
  const q = quiz != null ? quiz/10 : 0.3;
...
// pay-period helpers
function getPayPeriods(spending, nextPayday, monthlyPay) {
  if (!spending.length) return [];
  const pd = new Date(nextPayday);
  // build a list of period end-dates going back 6 months
  const periods = [];
  let end = new Date(pd);
  for (let i = 0; i < 6; i++) {
...
// ─── SVG Icons (consistent line style, 24px viewBox) ─────────────────────────
const Icon = ({ name, size=22, color="currentColor", strokeWidth=1.4 }) => {
  ...
  const paths = {
    ...
    spending: <><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z" fill="none" stroke={color} strokeWidth={strokeWidth}/><path d="M12 6v2m0 8v2M9 9.5h4.5a1.5 1.5 0 010 3h-3a1.5 1.5 0 000 3H15" fill="none" stroke={color} strokeWidth={strokeWidth} strokeLinecap="round"/></>,
...
// Controlled number input that allows full deletion
function NumInput({ value, onChange, style, ...rest }) {
  ...
  return (
    <input
      ...
      style={{
        fontFamily:T.sans, fontSize:15, border:`1.5px solid ${T.border}`,
        borderRadius:10, padding:"11px 14px", background:T.cream,
        color:T.ink, outline:"none", width:"100%", WebkitAppearance:"none", ...style
      }}
    />
  );
}
...
// Single button component — variant drives the look
const Btn = ({ variant="primary", style, children, icon, ...p }) => {
  const base = {
    fontFamily:T.sans, fontSize:14, fontWeight:500, border:"none",
  ...
  const v = {
    ...
    outline: { background:"transparent", color:T.ink, border:`1.5px solid ${T.border}` },
    ghost:   { background:"transparent", color:T.inkLight, padding:"8px 12px" },
    danger:  { background:"transparent", color:T.clay, border:`1.5px solid ${T.clay}` },
    sand:    { background:T.paper, color:T.ink, border:`1.5px solid ${T.border}` },
  };
  return <button {...p} style={{ ...base, ...v[variant], ...style }}>{icon && <Icon name={icon} size={16} color={variant==="primary"?T.cream:T.ink} />}{children}</button>;
};
...
const Popup = ({ open, onClose, title, children }) => (
  <>
    ...
    <div style={{
      ...
      background:T.cream, borderRadius:18, padding:"24px 20px",
...
export default function MeFirst() {
  ...
  // Hourly rate is always derived — never stored separately
  ...
  const [spending, setSpending] = useState([
    { id:1, category:"Groceries",     amount:180, date:"2026-02-01" },
    { id:2, category:"Transport",     amount:65,  date:"2026-02-03" },
    { id:3, category:"Dining",        amount:95,  date:"2026-02-07" },
    { id:4, category:"Beauty",        amount:45,  date:"2026-02-10" },
    { id:5, category:"Home",          amount:120, date:"2026-02-14" },
    { id:6, category:"Entertainment", amount:30,  date:"2026-02-17" },
    { id:7, category:"Clothing",      amount:210, date:"2026-02-20" },
  ]);
  ...
  const [budgetView, setBudgetView]     = useState("merged"); // kept for future use
  const [confirmClear, setConfirmClear] = useState(false);

  // form drafts
  const [draftSpend, setDraftSpend] = useState({ category:"Groceries", amount:"", date:new Date().toISOString().split("T")[0] });
  ...
  const daysLeft    = Math.max(Math.ceil((payday - today) / 86400000), 0);
  const periods     = getPayPeriods(spending, nextPayday, monthlyPay);
  ...
  const allSpend    = spending.reduce((a,b) => a+b.amount, 0);
  ...
  function addSpend() {
    ...
    setDraftSpend({ category:"Groceries", amount:"", date:new Date().toISOString().split("T")[0] });
    setSheet(null);
  }
  function addWish() {
    ...
    setDraftWish({ name:"", price:"", category:"Fashion" });
    setSheet(null);
  }
  function addGoal() {
    ...
    setDraftGoal({ name:"", target:"", current:"" });
    setSheet(null);
  }
  function saveGoal() {
    setGoals(g => g.map(x => x.id===editGoal.id ? editGoal : x));
    setEditGoal(null); setSheet(null);
  }
  function deleteGoal() {
    setGoals(g => g.filter(x => x.id!==editGoal.id));
    setEditGoal(null); setSheet(null);
  }
  function submitQuiz() {
    let score = 5;
  ...
  function clearAllData() {
    ...
    ["mf:goals","mf:wishlist","mf:spending","mf:budget","mf:pay","mf:hours","mf:payday"].forEach(k => {
    ...
    <div style={{ fontFamily:T.serif, background:T.cream, minHeight:"100dvh", paddingBottom:72, color:T.ink }}>
      ...
      <div style={{ padding:"20px 16px", maxWidth:480, margin:"0 auto" }}>
            ...
            {[...curPeriod.items].sort((a,b) => new Date(b.date)-new Date(a.date)).map(s => (
              <div key={s.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:T.white, borderRadius:14, padding:"12px 14px", boxShadow:`0 1px 8px rgba(46,37,32,0.06)` }}>
                <div style={{ display:"flex", gap:12, alignItems:"center" }}>
                  <div style={{ background:T.paper, borderRadius:10, width:40, height:40, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:T.sans, fontSize:10, color:T.inkLight, fontWeight:500, textAlign:"center", lineHeight:1.2, padding:4 }}>
        ...
        {tab === "savings" && (
          <div style={{ display:"grid", gap:12 }}>
            ...
            <Card style={{ background:T.paper }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                <div>
                  ...
                  <div style={{ fontSize:24, fontWeight:300, fontStyle:"italic", color:T.sage }}>{fmt(goals.reduce((a,g)=>a+g.current,0))}</div>
                </div>
                <div>
                  <Label>Total target</Label>
                  <div style={{ fontSize:24, fontWeight:300, fontStyle:"italic" }}>{fmt(goals.reduce((a,g)=>a+g.target,0))}</div>
                </div>
              </div>
            </Card>
            ...
            {goals.length === 0 && (
              <Card style={{ textAlign:"center", padding:40 }}>
                <div style={{ fontStyle:"italic", fontSize:16, opacity:0.4 }}>No savings goals yet</div>
              </Card>
            )}
              ...
              return (
                <Card key={g.id} onClick={() => { setEditGoal({...g}); setSheet("editGoal"); }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                    ...
                    <button style={{ background:"none", border:"none", cursor:"pointer", padding:4, opacity:0.35 }}>
          ...
          <div style={{ display:"grid", gap:12 }}>
            ...
            <Card style={{ background:T.paper, padding:"12px 16px" }}>
              <div style={{ fontFamily:T.sans, fontSize:13, color:T.inkLight }}>
                ...
                <span style={{ fontSize:11, opacity:0.6 }}> · based on {fmt(monthlyPay)} / {monthlyHours}h per month</span>
              </div>
            </Card>

            {wishlist.length === 0 && (
              <Card style={{ textAlign:"center", padding:40 }}>
                <div style={{ fontStyle:"italic", fontSize:16, opacity:0.4 }}>Your wishlist is empty</div>
              </Card>
            )}
        ...
        {tab === "budget" && (
          <div style={{ display:"grid", gap:12 }}>
            ...
            <Card style={{ background:T.ink, color:T.cream }}>
              ...
              <div style={{ fontFamily:T.sans, fontSize:10, opacity:0.3, marginTop:8 }}>
                Reality check uses: {curPeriod.label || "current period"}
              </div>
            </Card>

            {/* View toggle removed — side by side is the default */}

            {/* Helper function for actual spending per budget category */}
            {(() => {
              function getActual(b) {
                if (b.label==="Home & Bills") return curPeriod.items.filter(s=>s.category==="Home").reduce((a,x)=>a+x.amount,0);
                if (b.label==="Personal & Fun") return curPeriod.items.filter(s=>["Beauty","Entertainment","Clothing","Dining"].includes(s.category)).reduce((a,x)=>a+x.amount,0);
                return curPeriod.items.filter(s=>s.category===b.label.split(" ")[0]).reduce((a,x)=>a+x.amount,0);
              }
      ...
      <Sheet open={sheet==="addSpend"} onClose={() => setSheet(null)} title="Add expense">
        <div style={{ display:"grid", gap:14 }}>
          <Field label="Category"><Sel value={draftSpend.category} onChange={e=>setDraftSpend({...draftSpend,category:e.target.value})}>{SPEND_CATS.map(c=><option key={c}>{c}</option>)}</Sel></Field>
      ...
      {/* Settings */}
      <Sheet open={sheet==="settings"} onClose={() => setSheet(null)} title="Settings">
        <div style={{ display:"grid", gap:14 }}>
          ...
          <div style={{ marginTop:8, paddingTop:16, borderTop:`1px solid ${T.border}` }}>
            ...
            {!confirmClear ? (
              <Btn variant="danger" style={{ width:"100%" }} icon="trash" onClick={() => setConfirmClear(true)}>
                Clear all data
              </Btn>
            ) : (
              <div style={{ background:"#fff0ee", borderRadius:12, padding:16, display:"grid", gap:10 }}>
                <div style={{ fontFamily:T.sans, fontSize:13, color:T.clay, lineHeight:1.5 }}>
                  This will permanently delete all your spending, goals, and wishlist items. Your settings will also reset. <strong>This cannot be undone.</strong>
                </div>
          ...
          {budget.map((b,i) => (
            <div key={b.label}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                <Label>{b.label}</Label>
                <span style={{ fontFamily:T.sans, fontSize:11, color:T.inkLight }}>{fmt(monthlyPay*b.pct/100)}</span>
              </div>
              <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                ...
                <div style={{ fontFamily:T.sans, fontSize:11, color:T.inkLight, whiteSpace:"nowrap" }}>%</div>
              </div>
            </div>
          ))}
        ...
        {quizItem && (
          <div>
            ...
            <div style={{ display:"grid", gap:16, marginBottom:20 }}>
              {QUIZ_Q.map((q,i) => (
                <div key={i}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                    {["yes","no"].map(ans => (
                      <button key={ans} onClick={() => setQuizAns(a=>({...a,[i]:ans}))} style={{
                        padding:"10px 0", fontFamily:T.sans, fontSize:14, borderRadius:10, cursor:"pointer", transition:"all 0.15s",
            ...
            <Btn variant="primary" style={{ width:"100%", opacity:Object.keys(quizAns).length<QUIZ_Q.length?0.45:1 }}
              disabled={Object.keys(quizAns).length<QUIZ_Q.length}
              onClick={submitQuiz}>Get my verdict</Btn>
          </div>
        )}
      </Popup>
    </div>
